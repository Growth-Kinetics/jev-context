...
- **Eval gate.** `eval/` holds the labeled golden corpus (12 sessions, 19 skills) plus
  `expected.json`. Ratchet conditions, all must hold:
  - every labeled-relevant skill scores ≥ its recorded floor;
  - irrelevant-skill false-positive rate at the shipping threshold ≤ recorded ceiling (~5%);
  - changing a threshold requires re-running the eval and committing updated expectations
    in the same commit.
