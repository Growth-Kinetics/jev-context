...
| `golden-sessions.json` | 88-session corpus index (paths on this host) |
| `baseline-results.json` | recorded skill scores for the labeled 12-session golden subset |
| `expected.json` | labels, threshold policy, ratchet (floors + FP ceiling) |
| `fixtures/` | committed snapshots: skill catalog sizes, tool schema sizes, synthetic mini-session |
| `REPORT.md` / `REPORT-ALL.md` | committed report artifacts (golden 12 / all 88) |
...
node eval/run.ts --live --key-file /data/pi/agent/secrets/typesafe-jev.env
```
...
calibrated against the Jev usage observed in the 2026-09-18 probes (GOAL_2026-09-18-001
context). Jev spend is priced at $0.042/Mtok input. A real tokenizer can replace the constant
