...
import { runGate, labelMetrics, type BaselineEntry, type ExpectedFile } from "./check.ts";
...
const baseline: BaselineEntry[] = [
  { proj: "alpha", relevant: ["browser-use"], scores: { "browser-use": 0.92, spec: 0.3 }, errors: {} },
  { proj: "beta", relevant: [], scores: { "browser-use": 0.45, spec: 0.2 }, errors: {} },
];
...
test("gate fails when a labeled-relevant skill falls below its recorded floor", () => {
  const regressed = [{ ...baseline[0], scores: { ...baseline[0].scores, "browser-use": 0.61 } }, baseline[1]];
...
test("gate fails when the irrelevant false-positive rate exceeds the ceiling", () => {
  const noisy: BaselineEntry[] = [
    ...baseline,
    { proj: "gamma", relevant: [], scores: { "browser-use": 0.9, spec: 0.9 }, errors: {} },
  ];
...
test("gate fails loud when a floor references a missing session or skill", () => {
  const broken: ExpectedFile = {
    ...expected,
    ratchet: { floors: { "ghost/skill": 0.5 }, fp_ceiling: 1, threshold_at_record: 0.6 },
  };
  const failures = runGate(broken, baseline);
  assert.ok(failures.some((f) => f.check === "floor" && f.detail.includes("ghost")));
});

test("gate pins the threshold at record time: policy drift fails without a re-run", () => {
  const drifted: ExpectedFile = { ...expected, threshold_policy: { load: 0.5, top_k: 3, decay: 0.25 } };
...
test("gate demands a ratchet section before anything else", () => {
  const failures = runGate({ threshold_policy: expected.threshold_policy, labels: expected.labels }, baseline);
...
test("label metrics count judgments, FP at threshold, and loaded-but-not-labeled", () => {
  const hot: BaselineEntry[] = [
    baseline[0],
    { proj: "beta", relevant: [], scores: { "browser-use": 0.7, spec: 0.2 }, errors: {} },
  ];
  ...
  assert.deepEqual(m.loadedNotLabeled, [{ proj: "beta", skill: "browser-use" }]);
});
